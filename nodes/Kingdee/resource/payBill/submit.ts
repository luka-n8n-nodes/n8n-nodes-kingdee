...
const FORM_ID = 'AP_PAYBILL';

const operation: ResourceOperations = {
	name: '提交',
	value: 'submit',
	description: '提交付款单 (Submit)',
	action: '提交付款单',
	order: 2,
	options: [
		{
			...
			options: [
				{
					name: '按编码提交 (FNumber)',
					value: 'number',
					description: '使用单据编码 FNumber 字段提交',
				},
				{
					name: '按内码提交 (FID)',
					value: 'id',
					description: '使用表单内码 FID 字段提交',
				},
			],
			description: '选择提交付款单的方式',
		} as INodeProperties,
		{
			displayName: '付款单编码',
			name: 'numbers',
			...
			description: '付款单编码 FNumber，多个编码用逗号分隔，如：No1,No2,No3',
		} as INodeProperties,
		{
			displayName: '付款单内码',
			name: 'ids',
			...
			description: '付款单表单内码 FID，多个内码用逗号分隔，如：100001,100002,100003',
		} as INodeProperties,
		{
			displayName: '创建者组织内码',
			name: 'createOrgId',
	...
	call: async function (index) {
		...
		return RequestUtils.call.call(this, 'DynamicFormService', 'Submit', [FORM_ID, data]);
	},
};
