...
const FORM_ID = 'IV_PURCHASEIC';

const operation: ResourceOperations = {
	name: '查看',
	value: 'view',
	description: '查看采购增值税专用发票详情 (View)',
	action: '查看采购增值税专用发票',
	order: 1,
	options: [
		{
			...
			description: '选择查询采购增值税专用发票的方式',
		} as INodeProperties,
		{
			displayName: '采购增值税专用发票编码',
			name: 'number',
			type: 'string',
			default: '',
			required: true,
			...
			description: '采购增值税专用发票编码 FNumber，字符串类型',
		} as INodeProperties,
		{
			displayName: '采购增值税专用发票内码',
			name: 'id',
			type: 'string',
			default: '',
			required: true,
			...
			description: '采购增值税专用发票表单内码 FID，整数类型',
		} as INodeProperties,
		{
			displayName: '创建者组织内码',
			name: 'createOrgId',
