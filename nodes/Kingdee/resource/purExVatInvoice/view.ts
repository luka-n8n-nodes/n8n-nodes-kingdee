...
const FORM_ID = 'IV_PUREXVATIN';

const operation: ResourceOperations = {
	name: '查看',
	value: 'view',
	description: '查看进项费用增值税发票详情 (View)',
	action: '查看进项费用增值税发票',
	order: 1,
	options: [
		{
			...
			description: '选择查询进项费用增值税发票的方式',
		} as INodeProperties,
		{
			displayName: '进项费用增值税发票编码',
			name: 'number',
			type: 'string',
			default: '',
			required: true,
			...
			description: '进项费用增值税发票编码 FNumber，字符串类型',
		} as INodeProperties,
		{
			displayName: '进项费用增值税发票内码',
			name: 'id',
			type: 'string',
			default: '',
			required: true,
			...
			description: '进项费用增值税发票表单内码 FID，整数类型',
		} as INodeProperties,
		{
			displayName: '创建者组织内码',
			name: 'createOrgId',
