...
const operation: ResourceOperations = {
			...
			options: [
				{
					name: '按编码反禁用 (FNumber)',
					value: 'number',
					description: '使用用户编码 FNumber 字段反禁用',
				},
				{
					name: '按内码反禁用 (FUserID)',
					value: 'id',
					description: '使用用户内码 FUserID 字段反禁用',
				},
			],
			description: '选择反禁用系统用户的方式',
		} as INodeProperties,
		{
			displayName: '用户编码',
			name: 'numbers',
	...
	call: async function (index) {
		...
		// 调用金蝶 WebAPI 通用操作接口
		// 使用 ExecuteOperation 接口，opNumber 为 'Enable' 表示反禁用操作
		return RequestUtils.call.call(this, 'DynamicFormService', 'ExecuteOperation', [FORM_ID, 'Enable', data]);
	},
};
