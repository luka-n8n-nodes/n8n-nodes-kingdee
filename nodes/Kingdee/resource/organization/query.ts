...
const FORM_ID = 'ORG_Organizations';

const operation: ResourceOperations = {
	name: '查询列表',
	value: 'query',
	description: '查询组织机构列表 (ExecuteBillQuery)',
	action: '查询组织机构列表',
	order: 2,
	options: [
		...
		} as INodeProperties,
		{
			...
			default: '',
			description:
				"过滤条件，支持 SQL 语法。示例：FNumber = '100'、FName like '%总%'",
		} as INodeProperties,
		{
			displayName: '排序字段',
			name: 'orderString',
			...
			default: '',
			description:
				'排序字段，格式：字段名 ASC/DESC。示例：FNumber ASC（按编码升序）、FID DESC（按内码降序）',
		} as INodeProperties,
		queryOptions,
	],
