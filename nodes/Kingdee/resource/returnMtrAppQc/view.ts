...
const FORM_ID = 'QM_ReturnMtrAppQc';

const operation: ResourceOperations = {
	name: '查看',
	value: 'view',
	description: '查看生产退料请检单详情 (View)',
	action: '查看生产退料请检单',
	order: 1,
	options: [
		{
			...
			description: '选择查询生产退料请检单的方式',
		} as INodeProperties,
		{
			displayName: '生产退料请检单编码',
			name: 'number',
			type: 'string',
			default: '',
			required: true,
			...
			description: '生产退料请检单编码 FNumber，字符串类型',
		} as INodeProperties,
		{
			displayName: '生产退料请检单内码',
			name: 'id',
			type: 'string',
			default: '',
			required: true,
			...
			description: '生产退料请检单表单内码 FID，整数类型',
		} as INodeProperties,
		{
			displayName: '创建者组织内码',
			name: 'createOrgId',
