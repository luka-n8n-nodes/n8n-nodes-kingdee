...
const FORM_ID = 'CN_PAYAPPLY';

const operation: ResourceOperations = {
	name: '查询列表',
	value: 'query',
	description: '查询付款申请单列表 (ExecuteBillQuery)',
	action: '查询付款申请单列表',
	order: 2,
	options: [
		...
		paginationOptions.limit(2000),
		{
			...
			type: 'string',
			default: 'FID,FBillNo',
			description: '查询字段列表，多个字段用逗号分隔。示例：FID,FBillNo,FDate,FBillTypeID',
		} as INodeProperties,
		{
			displayName: '过滤条件',
			name: 'filterString',
			...
			default: '',
			description:
				"过滤条件，支持 SQL 语法。示例：FBillNo = 'FKSQ001'、FDocumentStatus = 'C'",
		} as INodeProperties,
		{
			displayName: '排序字段',
			name: 'orderString',
			...
			default: '',
			description:
				'排序字段，格式：字段名 ASC/DESC。示例：FBillNo ASC（按编码升序）、FID DESC（按内码降序）',
		} as INodeProperties,
		queryOptions,
	],
