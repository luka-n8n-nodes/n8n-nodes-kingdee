...
const operation: ResourceOperations = {
	name: '取消分配',
	value: 'cancelAllocate',
	description: '取消分配物料清单 (CancelAllocate)',
	action: '取消分配物料清单',
	order: 13,
	options: [
		{
			displayName: '物料清单内码列表',
			name: 'pkIds',
			type: 'string',
			default: '',
			required: true,
			description: '物料清单内码列表，多个内码用逗号分隔，如：100001,100002,100003',
		} as INodeProperties,
		{
	...
	call: async function (index) {
		...
		return RequestUtils.call.call(this, 'DynamicFormService', 'CancelAllocate', [FORM_ID, data]);
	},
};
