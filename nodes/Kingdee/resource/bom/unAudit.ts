...
const operation: ResourceOperations = {
			...
			options: [
				{
					name: '按编码反审核 (FNumber)',
					value: 'number',
					description: '使用单据编码 FNumber 字段反审核',
				},
				{
					name: '按内码反审核 (FID)',
					value: 'id',
					description: '使用表单内码 FID 字段反审核',
				},
			],
			description: '选择反审核物料清单的方式',
		} as INodeProperties,
		{
			displayName: '物料清单编码',
			name: 'numbers',
	...
	call: async function (index) {
		...
		return RequestUtils.call.call(this, 'DynamicFormService', 'UnAudit', [FORM_ID, data]);
	},
};
