...
const operation: ResourceOperations = {
			...
			options: [
				{
					name: '按编码禁用 (FNumber)',
					value: 'number',
					description: '使用单据编码 FNumber 字段禁用',
				},
				{
					name: '按内码禁用 (FID)',
					value: 'id',
					description: '使用表单内码 FID 字段禁用',
				},
			],
			description: '选择禁用物料清单的方式',
		} as INodeProperties,
		{
			displayName: '物料清单编码',
			name: 'numbers',
	...
	call: async function (index) {
		...
		return RequestUtils.call.call(this, 'DynamicFormService', 'Forbid', [FORM_ID, data]);
	},
};
