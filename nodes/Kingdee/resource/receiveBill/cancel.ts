...
const operation: ResourceOperations = {
			...
			options: [
				{
					name: '按编码作废 (FNumber)',
					value: 'number',
					description: '使用单据编码 FNumber 字段作废',
				},
				{
					name: '按内码作废 (FID)',
					value: 'id',
					description: '使用表单内码 FID 字段作废',
				},
			],
			description: '选择作废收款单的方式',
		} as INodeProperties,
		{
			displayName: '收款单编码',
			name: 'numbers',
	...
	call: async function (index) {
		...
		return RequestUtils.call.call(this, 'DynamicFormService', 'Cancel', [FORM_ID, data]);
	},
};
