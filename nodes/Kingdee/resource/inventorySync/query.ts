...
const FORM_ID = 'STK_InvUpdateToGY';

const operation: ResourceOperations = {
	name: '单据查询',
	value: 'query',
	description: '查询库存同步列表 (ExecuteBillQuery)',
	action: '查询库存同步列表',
	order: 1,
	options: [
		...
		paginationOptions.limit(2000),
		{
			...
			default: 'FID',
			description:
				'查询字段列表，多个字段用逗号分隔。示例：FID,FMaterialId,FStockOrgId',
		} as INodeProperties,
		{
			displayName: '过滤条件',
			name: 'filterString',
			...
			default: '',
			description:
				"过滤条件，支持 SQL 语法。示例：FStockOrgId.FNumber = '100'",
		} as INodeProperties,
		{
			displayName: '排序字段',
			name: 'orderString',
			...
			default: '',
			description:
				'排序字段，格式：字段名 ASC/DESC。示例：FID ASC',
		} as INodeProperties,
		queryOptions,
	],
	call: async function (index) {
		const returnAll = this.getNodeParameter('returnAll', index, false) as boolean;
		const fieldKeys = this.getNodeParameter('fieldKeys', index, 'FID') as string;
